...
contract SolnSquareVerifier is RealEstatePropertyToken {
    ...
    function addSolution(uint256 index, address addr1) internal {
        solutions.push(Solution(index, addr1));
        emit SolutionAdded(addr1);
    }

    // TODO Create a function to mint new NFT only after the solution has been verified
    //  - make sure the solution is unique (has not been used before)
    //  - make sure you handle metadata as well as tokenSuplly
    function mintNFT(Verifier.Proof calldata proof, uint256[2] calldata input) external returns(bool){
        ...
        uniqueSols[key] = msg.sender;
        addSolution(solutions.length, msg.sender);
        return this.mint(msg.sender, solutions.length);
    }
}
